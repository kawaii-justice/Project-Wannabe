...
1.  **本文分割 (`split_main_text` 関数 - 挙動変更):**
    *   UIの本文エリアのテキスト (`main_text`) が、内部的に3つの部分に分けられる概念となります。
        1.  **`main_part`**: 本文の末尾から4行目より前の部分。
            *   **本文の長さ制限:** この `main_part` は、ユーザーが設定画面で指定した「最大本文コンテキスト長 (`max_context_length`)」を超えないように、必要に応じて先頭から自動的にカットされます。これにより、本文が非常に長くなった場合でも、プロンプトのサイズが適切に管理され、参考情報などが失われるのを防ぎます。
