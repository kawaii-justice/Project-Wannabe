...
対応モデルでは、生成前の思考を分離して扱うことができます。Project Wannabe では、思考モードを有効にした生成結果に「思考」欄を表示します。

思考モードは、設定の **チャットテンプレモード** が `汎用` のときに使えます。リアルタイム提案では使用できません。

モデルや KoboldCpp の設定によって挙動が変わるため、うまく出ない場合は `設定` > `生成パラメータ設定...` の Thinking Template や prefill 設定を確認してください。
...
| 汎用 | それ以外のモデル、またはモデル側のチャットテンプレートを使いたい場合 |
...
### 2026-05-16 - UI更新・チャットテンプレモード・思考モード対応
...
* **生成候補カード:**
    * 生成結果をカードとして管理し、本文へ挿入、メモへ送る、コピー、削除がしやすくなりました。
    * 思考モード利用時は、思考欄を候補カード内に分けて表示します。
* **チャットテンプレモード:**
    * `旧wanabiシリーズ` と `汎用` を選べるようになりました。
    * 初回起動時に選択ダイアログを表示し、あとから設定画面で変更できます。
* **思考モード:**
    * 対応モデル向けに、思考出力の表示と prefill 設定を追加しました。
    * リアルタイム提案では思考モードは使用しません。
* **ドキュメント更新:**
