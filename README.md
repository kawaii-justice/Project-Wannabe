...
*   **リアルタイムで続きを提案（ベータ）:**
    *   執筆中のカーソル位置に、AIが続きの文章を薄い文字（ゴーストテキスト）でリアルタイムに提案します。
    *   **操作:** `Tab`キーで確定（採用）、`Esc`キーでキャンセル、`Ctrl+Space`で手動生成。
    *   **設定:**反応時間、動作モード（自動/手動）をカスタマイズ可能。「手動モード」では勝手に生成されず、`Ctrl+Space`を押した時だけ提案されます。
...
*   **表示設定:**
    ...
    *   **テーマ:** 現在はOS側の表示設定に追従しやすくするため、アプリ内のダークテーマ切り替えは一時的に非表示です。
...
### 2025-12-28 - 大規模アップデート🎉-新モデル・リアルタイム提案機能・検索機能の実装とコンテキスト管理の強化・コメントアウト


*   **[新モデル:Wanabi-Novelist-24B](https://huggingface.co/kawaimasa/Wanabi-Novelist-24B-GGUF)**
    *   ベースモデルの強化、コンテキストの増加、データ量、品質の向上を行い学習した過去最高の**[wanabi](https://huggingface.co/collections/kawaimasa/wanabi-683144d1bf44a0b922aa1609)モデル**です。
    *   変更点の詳細は[モデルカード](https://huggingface.co/kawaimasa/Wanabi-Novelist-24B-GGUF)をご確認ください。
    *  **⚠️ 重要: 新モデル「Wanabi-Novelist-24B」を使用するには本アップデートが必須です** モデル側のプロンプト仕様変更に対応するため、古いバージョンでは正しく動作しません。

*   **リアルタイムで続きを提案（ベータ）機能の実装:**
    *   執筆中のカーソル位置にAIが続きの文章を薄い文字（ゴーストテキスト）でリアルタイムに提案する機能を追加しました。
    *   `Tab`キーで確定、`Esc`キーでキャンセル、`Ctrl+Space`で手動生成が可能です。
    *   設定ダイアログから、生成長や反応時間、動作モード（自動/手動）を変更可能です。
    *  **⚠️ 重要: 本機能を快適に使うためにはVRAM内にコンテキストをすべて収め、プロンプトのトークン数がコンテキストを超えないようにする必要があります。**

*   **検索・置換機能（ベータ）の実装:**
    *   `Ctrl+F`で検索ダイアログを表示し、本文・メモ・詳細情報（タイトル等）を対象に検索・置換が可能です。正規表現もサポートしています。
    *   安全のため、検索中はリアルタイム提案機能が自動的にOFFになります。
    *   本機能はベータ版です。特に置換機能の使用時はバックアップを取るなど自衛をお願いしています。
*   **UI/UXの改善:**
    *   **ショートカット表示:** 画面下部に現在のモードで使用可能なショートカットキーを常時表示するようにしました。
    *   **設定ダイアログの整理:** 設定項目を整理し、「最大コンテキスト超過時の処理」などを折りたたみ式（Collapsible）に変更しました。
*   **本文の動的トークン数圧縮機能を追加:**
    *   KoboldCpp APIの `tokencount` と `true_max_context_length` を利用し、プロンプトがモデルの最大コンテキスト長を超えないように、本文の先頭部分を動的に削除する機能を追加しました。
    *   これにより、非常に長い小説を執筆する際に、重要な指示（詳細情報など）を維持しつつ、コンテキストオーバーによるエラーを回避します。
*   **コンテキスト管理設定のUIを改善:**
    *   生成パラメータ設定画面に「最大コンテキスト超過時の処理」セクションを追加。
    *   圧縮モードを以下の3つから選択できるようになりました。
        *   `本文をトークン数に基づき圧縮 (推奨)`: 上記の動的圧縮を行います。
        *   `最大本文文字数にトリム`: 従来通り、指定した文字数で本文の先頭をカットします。
        *   `何もしない (非推奨)`: コンテキスト超過チェックを無視します。
    *   設定項目の名称を「最大長」から「最大出力長」に変更し、機能をより分かりやすくしました。
*   **警告機能の強化:**
    *   動的圧縮後もコンテキスト長を超過する場合、エラーメッセージを表示して生成を中止する機能を追加しました。
    *   本文が圧縮され、AIに渡される文脈が著しく短くなった場合に、生成品質の低下を警告する機能を追加しました。（無限生成中はセッションごとに一度だけ表示されます。）
*   **プロンプト仕様の更新:**
    *   詳細情報欄のオーサーズノートを名称をより分かりやすい名称に変更しました。
    *   新しいモデルバージョンとの互換性向上のため、オーサーズノートのヘッダー名を `【この先の展開についての指示・メモ】` に変更するオプションを追加しました。（設定から変更可能）
    *   wanabi-24B, 12Bminiを使用する際は`設定`→`生成パラメータ設定`→`プロンプトフォーマット`から**レガシー**への変更が必須です。
*   **ドキュメント更新:**
    *   `README.md` および `prompt_logic.md` を最新の仕様に合わせて全面的に更新しました。
*   **コメントアウト・コンテキスト制御機能の実装:**
    *   本文や詳細情報内で、AIに送信しないテキストを指定できる機能を追加しました。
    *   **コメントアウト:** 行末コメント (`@//`) や範囲コメント (`@/* ... @*/`) を使って、メモ書きや一時的な除外が可能です。
    *   **範囲制御:** `@break` (または `@startpoint`) や `@endpoint` を使って、AIに読ませる文脈の範囲を強制的に指定できます。途中からの再開や、未来の展開を隠したままの執筆（挿入執筆）に便利です。
    *   詳細な仕様と使い方は [Dynamic Prompts & コンテキスト制御ガイド](docs/Dynamic_Prompts_Guide.md) を参照してください。
...
2.  **起動:** Windows では `run_app.bat` を実行します。ターミナルから起動する場合は `python main.py` でも起動できます。
3.  **初期設定 (初回):**
...
7.  **生成開始:**
    ...
    *   **無限生成:** メニューバーの `生成` > `無限生成 開始/停止 (F5)` を選択するか、ショートカットキーを押します。生成が開始され、出力候補エリアに結果がストリーミング表示されます。
8.  **生成停止 (無限生成中):** 再度 `F5` キーを押すか、メニューの `無限生成 開始/停止` を選択します。
9.  **テキスト転記:**
    *   出力候補カードの `本文へ挿入` ボタン、または出力エリア上部の `選択を本文へ` ボタンで本文エリアに挿入します。
    *   出力候補カードの `メモへ送る` ボタン、または出力エリア上部の `選択をメモへ` ボタンでメモエリアに追記します。
...
*   **表示設定:**
    ...
    *   **テーマ:** 現在はアプリ内のテーマ切り替えを非表示にしています。Windows側の表示補正が入る環境でも破綻しにくいよう、UI色はQtのパレットに基づいて調整しています。
...
2.  **ディレクトリ移動:**
    ...
    cd Project-Wannabe
    ```
3.  **Windows で簡単にセットアップする場合:**
    ```bat
    setup_venv.bat
    ```
    この bat は `venv` の作成、依存ライブラリのインストールを行い、完了後にそのまま起動するか確認します。
...
`run_app.bat` を実行してください。`venv` が存在しない場合は自動でセットアップしてから起動します。

テストを実行する場合は `run_tests.bat` を実行します。
...
画面下部にも現在のモードに応じたショートカットが表示されます。

| キー操作 | 機能 | 備考 |
| :--- | :--- | :--- |
| **Ctrl + G** | 単発生成 | 全モード有効 |
| **F5** | 無限生成 開始/停止 | 提案機能OFF時のみ |
| **Ctrl + F** | 検索・置換ダイアログ | 全モード有効 |
| **F3** / **Shift+F3** | 次を検索 / 前を検索 | |
| **Tab** | 提案を確定 | リアルタイム提案ON時 |
| **Esc** | 提案をキャンセル | リアルタイム提案ON時 |
| **Ctrl + Space** | 手動で提案を生成 | リアルタイム提案ON時 |
